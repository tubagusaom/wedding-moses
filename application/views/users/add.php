		  ...
		  $("#pegawai_id").combogrid({panelWidth: 450,idField:'id', textField:'nama_lengkap',mode: 'remote'});
			...
			if(newVal == 1 ){
				var cols = [[
					...
					{field:'nama_dealer',title:'Dealer',width:100},
					{field:'pemilik_dealer',title:'Pemilik',width:200}
				]];
				pgrid.datagrid({
					columns:cols,
					pagination: true,
					url: '<?php echo base_url() ?>pendaftaran_dealer/combogrid/1',
					fitColumns: true,
                    mode: 'remote',

				});
			} else if(newVal == 2) {
			     $("#pegawai_id").combogrid({panelWidth: 450,idField:'id', textField:'nama',mode: 'remote'});

				$("#pegawai_id").combogrid('clear');
				pgrid.datagrid({
					columns:[[
						{field:'id',hidden:true},
						{field:'nik',title:'Nik',width:300},
						{field:'nama',title:'Nama',width:300}
					]],
					pagination: true,
					url: '<?php echo base_url() ?>pegawai/combogrid',
					fitColumns: true,
                    mode: 'remote',

				});
			} else if(newVal == 3) {
			     $("#pegawai_id").combogrid({panelWidth: 450,idField:'id', textField:'nama_member',mode: 'remote'});

				$("#pegawai_id").combogrid('clear');
				pgrid.datagrid({
					columns:[[
						{field:'id',hidden:true},
						{field:'nama_member',title:'Customer',width:300},
					]],
					pagination: true,
					url: '<?php echo base_url() ?>member/combogrid',
					fitColumns: true,
                    mode: 'remote',
                    onChange: function(newVal, oldVal){
                        var g = $('#pegawai_id').combogrid('grid');
                        var r = g.datagrid('getSelected');	// get the selected row
                        $('#nama_member').val(r.nama_member);
                    }
				});
			} else {
