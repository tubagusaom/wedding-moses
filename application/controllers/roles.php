...
class Roles extends MY_Controller {
	...
	function combogrid($id = false) {
			...
			$params = array('_return' => 'data');
			if (isset($_POST['q'])) {
					$where['nama_peran LIKE'] = "%" . $this->input->post('q') . "%";
			}
			...
			$rows = isset($where) ? $this->Role_Model->order_by($order, $order_criteria, $_order_escape)->get_many_by($where) : $this->Role_Model->order_by($order, $order_criteria, $_order_escape)->get_all();
			$data['rows'] = $this->Role_Model->get_selected()->data_formatter($rows);
			echo json_encode($data);
	}
