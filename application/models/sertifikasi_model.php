...
class Sertifikasi_Model extends MY_Model {
    ...
    function get_detail_skema($id){
        $this->db->select('a.id, c.skema, c.kode_skema, c.link_download, c.description, b.unit_kompetensi, b.id_unit_kompetensi');
    ...
    function get_row_skema($kode_tbl,$id){
        
        $this->db->where('id',$id);
        return $this->db->get($kode_tbl.'skema')->row();
    }
    public function get_all_asesor($perpage, $offset,$search="") {
        if($search ==""){
			$this->db->select('a.*, b.skema');
			$this->db->from(kode_tbl().'users a');
			$this->db->join(kode_tbl().'skema b', 'a.skema_id = b.id', 'left');
            //$this->db->order_by('id', 'ASC');
            $this->db->limit($perpage);
            $this->db->offset($offset);
    ...
    public function get_all_asesi($perpage, $offset,$keyword="") {
        ...
        if($keyword ==""){
            ...
            $this->db->offset($offset);
            $this->db->select('a.*,b.skema');
            $this->db->from('lsp304_asesi a');
            $this->db->join('lsp304_skema b','a.skema_sertifikasi = b.id');
            $this->db->where('a.rekomendasi_asesor','1');
	    $this->db->where('a.terbitkan_sertifikat','on');
            ...
            $this->db->or_like('b.skema', $keyword);
    ...
    public function nama_sekolah() {
            ...
            $query = $this->db->get(kode_tbl().'psertifikasi');
        
        return $query->result();
    }
    public function count_peserta_uji(){
        return $this->db->count_all(kode_tbl().'asesi');
    }
    public function get_all_peserta_uji($perpage, $offset,$search="",$users="",$nis="",$asal_sekolah="") {
        if($search ==""){
           
            $this->db->select('e.users,b.nama_lengkap,b.no_identitas,b.organisasi,b.pra_asesmen_checked,b.jadwal_id,b.rekomendasi_description,b.skema_sertifikasi, c.skema, d.tanggal');
            $this->db->from(kode_tbl().'asesi b');
            $this->db->join(kode_tbl().'skema c', 'b.skema_sertifikasi = c.id');
            $this->db->join(kode_tbl().'jadual_asesmen d', 'b.jadwal_id = d.id', 'LEFT');
            $this->db->join(kode_tbl().'users e', 'b.id_asesor = e.id', 'LEFT');
            ...
            $this->db->join(kode_tbl().'users e', 'b.id_asesor = e.id', 'LEFT');
            if($users!=""){
                $this->db->like('b.nama_lengkap', $users);
            }
            if($nis!=""){
                $this->db->like('b.no_identitas', $nis);
            }
            if($asal_sekolah!=""){
                $this->db->where('b.organisasi', $asal_sekolah);    
            }
            $this->db->order_by('b.id', 'DESC');
            $this->db->limit($perpage);
            $this->db->offset($offset);
    ...
    function peserta_uji_search($users,$nis,$asal_sekolah){
            $this->db->select('b.nama_lengkap,b.pra_asesmen_checked,b.jadwal_id,b.rekomendasi_description,a.*');
            $this->db->from(kode_tbl().'psertifikasi a');
            $this->db->join(kode_tbl().'asesi b','b.nama_lengkap=a.users','LEFT');
            if($users!=""){
                $this->db->like('a.users', $users);
            }
            if($nis!=""){
                $this->db->like('a.nis', $nis);
            }
            if($asal_sekolah!=""){
                $this->db->where('a.asal_sekolah', $asal_sekolah);    
            }
    ...
    public function riwayat_sertifikat($id){
        $this->db->select('a.*,b.skema');
        $this->db->from(kode_tbl().'asesi a');
        $this->db->join(kode_tbl().'skema b','a.skema_sertifikasi=b.id');
        $this->db->where('id_users',$id);
    ...
    function detail_sertifikasi_jadwal($id,$id_users){
        ...
        $this->db->join(kode_tbl().'tuk e','b.id_tuk=e.id');
    ...
    function jadwal_uji_kompetensi(){
        ...
        return $this->db->get(kode_tbl().'jadual_asesmen')->result();
    }
    ...
    function unit_by_kode($kode){
        $this->db->select('a.*');
        $this->db->from(kode_tbl().'unit_kompetensi a');
        $this->db->where('a.id_unit_kompetensi', $kode);
        $unit_mak02 = $this->db->get()->row();      
        return $unit_mak02;
    }        
}
