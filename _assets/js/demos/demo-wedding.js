...
(function( $ ) {
	...
	*/
	$('#guestbookSendMessage').validate({
		submitHandler: function(form) {
			...
			$.ajax({
				...
				data: {
					id_pengunjung: $form.find('#idPengunjung').val(),
					...
					confirm: $form.find('#guestbookConfirm').val()
				}
			}).always(function(data, textStatus, jqXHR) {

				$xaom = "tera_byte";
				// $xaom = base_url + "application/templates/bootstraps/ucapan.php";

				// $dataRespon = data.response;
				// alert(data);
				console.log($xaom);
		      ...
		      icon: 'success',
		      title: '<strong>Terkirim!</strong> Terima kasih atas doanya.',
		      ...
		      timer: 2000
				...
				$("#divDoa").load(" #divDoa > *");
				$("#guestbookSendMessage").load(" #guestbookSendMessage > *");
				// $("#divDoa").load(window.location.href + " #guestbook" );
	...
	$('#guestbookSendMessageXxx').validate({
		submitHandler: function(form) {
